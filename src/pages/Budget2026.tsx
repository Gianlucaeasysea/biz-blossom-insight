...
// Shopify collection → budget product (same mapping as CollectionBreakdown)
...
export default function Budget2026() {
  ...
  // ── Shopify orders for the full budget year ──────────────────────────────
  ...
  // ── Shopify Analytics YTD summary — canonical B2C net sales (same as Index.tsx) ──
  ...
  // ── B2C net scale factor (identical logic to Index.tsx) ─────────────────
  //    Aligns order-level netAmount totals to Shopify Analytics ground truth
  const b2cNetScaleFactor = useMemo(() => {
    if (!ytdSalesSummary?.netSales) return 1;
    const rawYTDNet = shopifyOrders
      .filter(o => o.customerType === 'B2C')
      .reduce((s, o) => s + (o.netAmount ?? o.totalAmount), 0);
    return rawYTDNet > 0 ? ytdSalesSummary.netSales / rawYTDNet : 1;
  }, [shopifyOrders, ytdSalesSummary]);

  // ── Per-month, per-product raw actuals from B2C orders ───────────────────
  //    Same proportional distribution formula as getB2CCollectionBreakdown()
    ...
    shopifyOrders
      ...
      .forEach(order => {
        ...
        order.products.forEach(item => {
          ...
          if (!prod) return;
          // Proportional net allocation (same as getB2CCollectionBreakdown)
  ...
  // ── Apply b2cNetScaleFactor → scaled actuals match Shopify Analytics ─────
  const actuals = useMemo<Record<string, number[]>>(() => {
    const scaled: Record<string, number[]> = {};
    RAW.products.forEach(p => {
      scaled[p.name] = (rawActuals[p.name] ?? new Array(12).fill(0))
        .map(v => Math.round(v * b2cNetScaleFactor * 100) / 100);
    });
    return scaled;
  }, [rawActuals, b2cNetScaleFactor]);

  // ── 2026 monthly targets per product ─────────────────────────────────────
  ...
  // Canonical YTD B2C net sales: prefer Shopify Analytics (same as Index.tsx)
  ...
  // ── Which months are "past" (have actual data) ───────────────────────────
  const now = new Date();
  ...
  const advChart   = MONTHS.map((month, i) => ({
    month, '2025 Actual': RAW.advTotals[i], '2026 Budget': advBudget[i],
  }));

  // ── MER chart ────────────────────────────────────────────────────────────
  const merChart = MONTHS.map((month, i) => ({ month, '2025 MER': RAW.mer[i] }));
    ...
    <div className="min-h-screen bg-background p-4 sm:p-6">
      <div className="max-w-[1520px] mx-auto space-y-5">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="text-[11px] text-muted-foreground mt-0.5">
              B2C Net Sales — targets &amp; YTD performance · Easysea
              {ytdSalesSummary?.source === 'shopify_analytics' &&
                <span className="ml-2 text-primary/70">· Shopify Analytics</span>}
            </p>
          </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {([
            { label: '2026 Total Target',  value: fmtEur(totalTarget), color: 'hsl(168,70%,42%)',  icon: <Target     className="w-3.5 h-3.5" /> },
            { label: '2026 YTD Net Sales', value: fmtEur(ytdActual),   color: 'hsl(215,85%,55%)', icon: <TrendingUp className="w-3.5 h-3.5" /> },
            { label: '% Achieved',         value: `${pctDone.toFixed(1)}%`, color: pctDone >= 100 ? 'hsl(168,70%,42%)' : 'hsl(42,96%,48%)', icon: <Zap className="w-3.5 h-3.5" /> },
            { label: '2025 Reference',     value: fmtEur(total2025),   color: 'hsl(220,15%,55%)', icon: <CalIcon    className="w-3.5 h-3.5" /> },
          ] as const).map(k => (
        ...
        <div className="chart-container">
          <p className="section-label mb-3">Monthly B2C Net Sales — Actual vs Budget 2026</p>
          <div className="text-[10px] text-muted-foreground mb-3 flex flex-wrap gap-4">
            <span className="flex items-center gap-1.5">
              <span className="inline-block w-3 h-3 rounded-sm bg-white/20" />Solid = 2026 YTD Actual (scaled to Shopify Analytics)
            </span>
            <span className="flex items-center gap-1.5">
              <span className="inline-block w-3 h-3 rounded-sm bg-white/10 border border-white/20" />Faded = 2026 Budget
            </span>
            <span className="flex items-center gap-1.5">
              <span className="inline-block w-8 border-t-2 border-dashed border-muted-foreground/50" />2025 Reference
            </span>
          </div>
          <ResponsiveContainer width="100%" height={320}>
            <ComposedChart data={monthlyChart} margin={{ top: 4, right: 8, left: 0, bottom: 0 }}>
              ...
              {RAW.products.map(p => (
                <Bar key={`${p.name}_actual`} dataKey={`${p.name}_actual`} name={p.name}
                  stackId="actual" fill={PRODUCT_COLORS[p.name]} fillOpacity={0.9} radius={0} />
              ))}
              {RAW.products.map(p => (
              ...
              <Line dataKey="ref2025" name="2025 Reference" type="monotone"
                stroke="hsl(220,15%,55%)" strokeWidth={1.5} strokeDasharray="4 3" dot={false} legendType="plainline" />
              <Line dataKey="targetLine" name="2026 Budget Total" type="monotone"
        ...
        <div className="chart-container overflow-x-auto">
          <div className="flex items-center justify-between mb-3">
            <p className="section-label">Per-Product Detail — B2C Net Sales</p>
            <span className="text-[10px] text-muted-foreground">
              Past months: <span className="text-foreground font-semibold">actual</span> / <span className="text-muted-foreground">budget</span> · Future months: budget
            </span>
          </div>
          <table className="data-table w-full">
            <thead>
              <tr>
                ...
                <th className="text-center">MER</th>
                <th className="text-left">Notes</th>
                {MONTHS.map(m => (
                  <th key={m} className="text-right text-[9px] min-w-[58px]">{m}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {RAW.products.map(p => {
                const ytdProd = (actuals[p.name] ?? []).reduce((s, v) => s + v, 0);
                const tot25   = p.sales.reduce((s, v) => s + v, 0);
                const rem     = Math.max(0, p.target - ytdProd);
                const pct     = p.target > 0 ? Math.min(100, (ytdProd / p.target) * 100) : 0;
                return (
                  <tr key={p.name}>
                    <td>
                    ...
                    <td className="text-right font-mono text-xs font-semibold" style={{ color: PRODUCT_COLORS[p.name] }}>{fmtEur(ytdProd)}</td>
                    ...
                    <td className="text-xs text-muted-foreground max-w-[100px]">{p.notes || '—'}</td>

                    {MONTHS.map((_, i) => {
                      const actual = actuals[p.name]?.[i] ?? 0;
                      const budget = targets[p.name]?.[i] ?? 0;
                      const isPast = i <= currentMonth;

                      return (
                        <td key={i} className="text-right font-mono min-w-[58px] py-1.5">
                          {isPast ? (
                            /* Past month: actual (colored) + budget comparison below */
                            <div className="flex flex-col items-end gap-0.5">
                              <span className="text-[11px] font-semibold" style={{ color: actual > 0 ? PRODUCT_COLORS[p.name] : undefined }}>
                            ...
                            /* Future month: budget only */
                            <span className="text-[10px] text-muted-foreground/50">
                              {budget > 0 ? fmtK(budget) : '—'}
                            </span>
                          )}
                        </td>
                      );
            ...
            <tfoot>
              <tr className="border-t-2 border-border font-semibold text-xs">
                ...
                {/* YTD total uses canonical Shopify Analytics net sales */}
                <td className="text-right font-mono font-bold" style={{ color: 'hsl(168,70%,42%)' }}>{fmtEur(ytdActual)}</td>
                <td className="text-right font-mono">{fmtEur(Math.max(0, totalTarget - ytdActual))}</td>
                <td className="text-center">
                  <div className="flex items-center gap-1.5">
                    <div className="flex-1 h-1.5 rounded-full bg-muted/50">
                      <div className="h-1.5 rounded-full bg-primary transition-all" style={{ width: `${Math.min(100, pctDone)}%` }} />
                    </div>
                ...
                {MONTHS.map((_, i) => {
                  ...
                  const b = RAW.products.reduce((s, p) => s + (targets[p.name]?.[i] ?? 0), 0);
                  const isPast = i <= currentMonth;
                  return (
                    <td key={i} className="text-right font-mono py-1.5">
                      {isPast ? (
                        <div className="flex flex-col items-end gap-0.5">
                          <span className="text-[11px]">{fmtK(a)}</span>
                          <span className="text-[9px] text-muted-foreground/60">{fmtK(b)}</span>
                        </div>
                      ) : (
                        <span className="text-[10px] text-muted-foreground/50">{fmtK(b)}</span>
                      )}
                    </td>
                  );
                })}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          <div className="chart-container">
            ...
            <div className="flex gap-4 mb-2 text-[10px] text-muted-foreground">
              <span>2025 total: {fmtEur(advTot2025)}</span>
          ...
          <div className="chart-container">
            <p className="section-label mb-3">MER — 2025 Actual &amp; 2026 Targets per Product</p>
            ...
            <ResponsiveContainer width="100%" height={220}>
              <LineChart data={merChart} margin={{ top: 4, right: 8, left: 0, bottom: 0 }}>
                ...
                <ReferenceLine y={2.5} stroke="hsl(42,96%,48%)" strokeDasharray="4 3" strokeWidth={1.5}
                  label={{ value: 'target 2.5x', fill: 'hsl(42,96%,48%)', fontSize: 9, position: 'right' }} />
                <Line dataKey="2025 MER" type="monotone" stroke="hsl(168,70%,42%)" strokeWidth={2}
                  dot={{ r: 3, fill: 'hsl(168,70%,42%)' }} />
              </LineChart>
            </ResponsiveContainer>
          </div>
        </div>
        ...
        <div className="chart-container overflow-x-auto">
          ...
          <table className="data-table w-full">
            <thead>
              <tr>{MONTHS.map(m => <th key={m} className="text-center">{m}</th>)}</tr>
            </thead>
            <tbody>
              <tr>
                    ...
                    <div className="flex flex-col items-center gap-1">
                      ...
                      {c.initiative !== '—' && (
                        <span className={`inline-block px-1.5 py-0.5 rounded text-[9px] font-bold ${
                          ...
                          c.weight >= 1.2 ? 'bg-yellow-500/20 text-yellow-400' :
