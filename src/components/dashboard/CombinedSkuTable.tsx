  ...
  return (
    <div className="chart-container">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4">
        <h3 className="text-sm font-semibold">Combined B2C + B2B SKU Detail</h3>
        ...
        <table className="data-table">
          ...
          <tbody>
            {filtered.map(r => (
              <tr key={r.sku}>
                ...
                <td className="text-right font-mono text-xs">{fmt(r.b2cValue)}</td>
                <td className="text-right font-mono text-xs">{fmt(r.b2bValue)}</td>
          ...
          {filtered.length > 0 && (
            <tfoot><tr className="border-t-2 border-border font-semibold text-xs">
              ...
              <td className="text-right font-mono">{fmt(filtered.reduce((s, r) => s + r.totalValue, 0))}</td>
            </tr></tfoot>
          )}
        </table>
      </div>
