...
export function B2CSkuTable({ data, allProductNames = [], allSkus = [] }: { data: SkuRow[]; allProductNames?: string[]; allSkus?: string[] }) {
  const [search, setSearch] = useState('');
  const [selectedProduct, setSelectedProduct] = useState('');
  const [selectedSku, setSelectedSku] = useState('');
  const [sortField, setSortField] = useState<SortField>('netSalesTotal');
    ...
    <div className="chart-container">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-3">
        <div className="flex items-center gap-2">
          <span className="badge-b2c inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold tracking-wider uppercase">B2C</span>
          <h3 className="text-sm font-semibold accent-b2c">B2C SKU Detail</h3>
        </div>
          ...
          <tbody>
            {filtered.map(r => (
              <tr key={r.sku}>
                ...
                <td className="text-right font-mono text-xs">{fmt(r.netSalesTotal)}</td>
