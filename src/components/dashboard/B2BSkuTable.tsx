...
interface B2BSkuTableProps {
  data: SkuRow[];
}

type SortField = 'sku' | 'name' | 'fatturato' | 'ordiniRaccolti' | 'ordiniPagati';

export function B2BSkuTable({ data }: B2BSkuTableProps) {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }} className="chart-container">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div>
          <h3 className="text-lg font-semibold">Dettaglio B2B per SKU</h3>
        ...
        <table className="data-table">
          <thead>
            <tr>
              ...
              <th className="text-right"><SortBtn field="fatturato">Fatturato B2B</SortBtn></th>
              <th className="text-right"><SortBtn field="ordiniRaccolti">Ordini Raccolti B2B</SortBtn></th>
              <th className="text-right"><SortBtn field="ordiniPagati">Ordini Pagati B2B</SortBtn></th>
            </tr>
          </thead>
          <tbody>
            {filtered.map(row => (
              <tr key={row.sku}>
                ...
                <td className="text-right font-medium">{fmt(row.ordiniPagati)}</td>
              </tr>
            ))}
            {filtered.length === 0 && (
              <tr><td colSpan={5} className="text-center text-muted-foreground py-8">Nessun dato disponibile</td></tr>
            )}
          </tbody>
          {filtered.length > 0 && (
            <tfoot>
              <tr className="border-t-2 border-border font-semibold">
                ...
                <td className="text-right">{fmt(filtered.reduce((s, r) => s + r.ordiniPagati, 0))}</td>
              </tr>
            </tfoot>
          )}
        </table>
