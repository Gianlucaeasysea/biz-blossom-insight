...
export function SalesTrendChart({ orders, dateRange }: SalesTrendChartProps) {
  ...
  const { chartData, keys, colors } = useMemo(() => {
    ...
    orders.forEach(o => {
      if (viewMode === 'channel') {
        keySet.add(o.channel || 'Altro');
      } else {
        o.products.forEach(p => keySet.add(p.name));
      }
    });
    ...
    const chartData = intervals.map(interval => {
      ...
      keys.forEach(key => {
        if (viewMode === 'channel') {
          ...
          row[key] = intervalOrders.reduce((sum, o) =>
            sum + o.products.filter(p => p.name === key).reduce((ps, p) => ps + p.totalPrice, 0), 0);
        }
      });
      return row;
    });
    ...
    const palette = [
      'hsl(190, 100%, 50%)', 'hsl(270, 60%, 55%)', 'hsl(160, 60%, 45%)',
      'hsl(40, 90%, 55%)', 'hsl(0, 70%, 55%)', 'hsl(210, 70%, 55%)',
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-5">
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            Per {viewMode === 'channel' ? 'canale' : 'prodotto'}
          </p>
        </div>
